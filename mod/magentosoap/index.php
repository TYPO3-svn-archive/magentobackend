...
$LANG->includeLLFile('EXT:magentoconnect/mod1/locallang.xml');
...
class  tx_magentoconnect_module1 extends t3lib_SCbase {
				...
				function main()	{
					...
					$this->connect = unserialize($GLOBALS['TYPO3_CONF_VARS']['EXT']['extConf']['magentoconnect']);		

					if (($this->connect['username']!='' && $this->connect['password']!='' && $this->connect['url']!='') || ($this->id && $access) || ($BE_USER->user['admin'] && !$this->id))	{
				...
				function getAllProducts() {
					...
					$countries = $this->call('category.tree');
					
					$content .= $this->doc->section('SessionID:', t3lib_div::view_array($sessionId), 0, 2);
					$content .= $this->doc->section('Countries:', t3lib_div::view_array($countries), 0, 2);
					#$content .= $this->doc->section('Countries:', $tree->tree($countries, '', ''), 0, 2);
			 ...
			 */			  
			  ...
			  function disconnect()	{
					...
					$this->client = $this->sessionId = NULL;
			  }
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/magento/mod/magentosoap/index.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/magento/od/magentobsoap/index.php']);
}
